...
const herokuBackendUrl =
  import.meta.env.VITE_BACKEND_URL || 'https://memory-breakout.herokuapp.com'; // Heroku production URL

if (import.meta.env.DEV) {
  socket = io(localBackendUrl, {});
  console.log(
    `Socket.IO (Frontend): Verbinde mit lokalem Dev-Server: ${localBackendUrl}`
  );
} else {
  socket = io(herokuBackendUrl);
  console.log(
    `Socket.IO (Frontend): Verbinde mit Heroku-Backend: ${herokuBackendUrl}`
  );
}
